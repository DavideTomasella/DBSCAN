...
% isLeaf      -  1 = node is leaf
%                0 = node has 2 children, is an internal node
...
tree_cell(assigned_nn).isLeaf=0; 
...
if n==1
    ...
    tree_cell(assigned_nn).isLeaf=1;
...
if (tree_cell(assigned_nn).isLeaf==1)
    ...
    if (tree_cell(assigned_nn).numpoints>2)
        ...
        if(size(X,1)>size(i,1))
            tree_cell(assigned_nn).right = kd_buildtree(X(~(X(:, tree_cell(assigned_nn).splitdim)<=median_value), :),plot_stuff,assigned_nn,split_dimension);
        else
            tree_cell(assigned_nn).right =[];
        end
    else
